...
// FIX: Ubah MaxSlots menjadi 20 agar sesuai Config.lua
        ...
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    // --- FIX SERIAL NUMBER START ---
                    var myPhone = "000000";
                    var mySerial = "Unknown";
                    
                    if (QB.Phone.Data.PlayerData.charinfo) {
                        myPhone = QB.Phone.Data.PlayerData.charinfo.phone;
                    }
                    
                    // Gunakan ID Player atau Nomor HP sebagai ganti Serial Number
                    mySerial = "ESX-" + myPhone; 

                    $("#myPhoneNumber").text(myPhone);
                    $("#mySerialNumber").text(mySerial);
                    // --- FIX SERIAL NUMBER END ---
...
QB.Phone.Functions.Open = function(data) {
    // --- FITUR BARU: RESET KE HOME SAAT DIBUKA ---
    // 1. Sembunyikan aplikasi yang sedang aktif (jika ada)
    if (QB.Phone.Data.currentApplication !== null) {
        QB.Phone.Functions.ToggleApp(QB.Phone.Data.currentApplication, "none");
        QB.Phone.Data.currentApplication = null;
    }
    
    // 2. Kembalikan posisi container aplikasi ke atas (tersembunyi)
    $('.phone-application-container').css('top', '-160%');
    
    // 3. Kembalikan warna text header ke putih (default)
    QB.Phone.Functions.HeaderTextColor("white", 0);
    // ---------------------------------------------
